...
export default function LoginScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      Alert.alert('Login failed', e.response?.data?.message || 'Check your credentials and backend connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Screen contentStyle={styles.page}>
      <View style={styles.hero}>
        ...
        <Text style={styles.subtitle}>Vehicle lookup and dealer alert system</Text>
      </View>
      <View style={styles.card}>
        ...
        <Text style={styles.note}>Accounts are created by an Admin. There is no self-registration.</Text>
      </View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: {backgroundColor: '#fff', padding: 18, borderRadius: 22, gap: 14, borderWidth: 1, borderColor: colors.border},
