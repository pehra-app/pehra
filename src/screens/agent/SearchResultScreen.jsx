...
export default function SearchResultScreen({ route, navigation }) {
  ...
  if (!result.found) {
    return (
      <Screen contentStyle={styles.center}>
        <View style={[styles.circle, { backgroundColor: '#EEF2F7' }]}>
          <Text style={styles.symbol}>?</Text>
        </View>
        <Text style={styles.title}>No record found</Text>
        <Text style={styles.sub}>
          No vehicle matches “{result.searchedValue}”.
        </Text>
  ...
  return (
    <Screen contentStyle={styles.center}>
      {/* Clear result UI is disabled; search returns wanted vehicles only.
      ...
      */}
      <View style={[styles.circle, { backgroundColor: colors.dangerSoft }]}>
        <Text style={[styles.symbol, { color: colors.danger }]}>!</Text>
      </View>
      ...
      {wanted && (
        <Text style={styles.alertText}>
          Dealer alert has been created automatically.
        </Text>
      )}
      <AppButton
