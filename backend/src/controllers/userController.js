...
export async function listUsers(req, res) {
  const users = await User.find({role: {$in: ['DEALER', 'AGENT']}})
...
export async function createUser(req, res) {
  ...
  if (!name || !email || !password || !['DEALER', 'AGENT'].includes(role)) {
    return res.status(400).json({message: 'Name, email, password and Dealer/Agent role are required.'});
  }
...
export async function setUserStatus(req, res) {
  const user = await User.findOne({_id: req.params.id, role: {$in: ['DEALER', 'AGENT']}});
